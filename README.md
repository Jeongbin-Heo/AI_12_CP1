...
* **기본 모델** : `XGBoost`
* **평가 지표** : `F1 Score` (Recall과 Precision의 수치를 적절히 조합해 종합적인 성능 평가)
...
## :six: 한계점
* **캐글 데이터셋 사용** : 구매이력과 같은 개인정보는 크롤링을 통해 수집하는 것이 부적절하다고 판단하여 캐글에 공개되어 있는 장바구니 데이터 사용이 불가피하였음  
* **데이터 손실** : 불균형 데이터를 Undersampling 하는 과정에서 데이터 제거로 인한 정보가 손실됨  
* **서비스 배포 실패** : 추후 배포를 다시 시도할 필요가 존재함
